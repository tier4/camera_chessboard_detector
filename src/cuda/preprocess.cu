...
__global__ void bgr2gray_f_k(float *gray, const unsigned char *src, int w, int h, int channels)
{
...
__global__ void gauss_h_k(float *dst, const float *src, const float *k, int w, int h)
{
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  if (x >= w || y >= h) return;
...
__global__ void gauss_v_k(float *dst, const float *src, const float *k, int w, int h)
{
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  if (x >= w || y >= h) return;
...
__global__ void normalize_k(float *dst, const float *src, float mn, float inv_range, int n)
{
...
void CudaPreprocessor::ensureKernel()
{
  ...
  NVCHK(cudaMemcpy(d_kernel_, wf, kGaussKsize * sizeof(float), cudaMemcpyHostToDevice));
  kernel_ready_ = true;
}

const GpuImagePtr &CudaPreprocessor::run(
  const unsigned char *h_src, int width, int height, int channels
)
{
  ensureKernel();
  ...
  gauss_h_k<<<grid, block>>>(tmp_->data(), gray_->data(), d_kernel_, width, height);
  gauss_v_k<<<grid, block>>>(blur_->data(), tmp_->data(), d_kernel_, width, height);
  ...
  const int tb = 256;
  normalize_k<<<(total + tb - 1) / tb, tb>>>(norm_->data(), blur_->data(), mn, inv_range, total);
  return norm_;
}
