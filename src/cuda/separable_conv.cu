...
__global__ void sep_conv_h_kernel(float * __restrict__ out,
...
__global__ void sep_conv_v_accum_kernel(float * __restrict__ out,
                                        const float * __restrict__ in,
...
void decomposeKernel(const camera_chessboard_detector::CpuKernelF32 &kernel,
                     int requested_rank,
                     std::vector<std::vector<float>> &u_out,
  ...
  Eigen::JacobiSVD<Eigen::MatrixXf> svd(K, Eigen::ComputeFullU |
                                              Eigen::ComputeFullV);
  ...
  const int rank = (requested_rank <= 0) ? max_rank
                                          : std::min(requested_rank, max_rank);
...
void CudaSeparableConvolver::release_() {
  for (auto *p : d_u_) if (p) cudaFree(p);
  for (auto *p : d_v_) if (p) cudaFree(p);
  d_u_.clear();
  d_v_.clear();
}
...
void CudaSeparableConvolver::set_kernel(
  ...
  for (int k = 0; k < rank_; ++k) {
    ...
    NVCHK(cudaMemcpy(d_u_[k], u_components[k].data(),
                     S * sizeof(float), cudaMemcpyHostToDevice));
    NVCHK(cudaMemcpy(d_v_[k], v_components[k].data(),
                     S * sizeof(float), cudaMemcpyHostToDevice));
  }
}

void CudaSeparableConvolver::convolve(const GpuImagePtr &image,
                                      GpuImagePtr &output) {
  ...
  for (int k = 0; k < rank_; ++k) {
    sep_conv_h_kernel<<<grid, block, 0, stream_>>>(
        tmp_->data(), image->data(), d_v_[k], W, H, R);
