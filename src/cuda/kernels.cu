...
// CUDA port of the libcbdetect checkerboard corner detector of A. Geiger et al.
...
__global__ void conv2dShared(
  float *output, const float *input, const float *kernel, int H, int W, int S
)
{
#ifdef CONV_SHARED
  ...
  if (threadIdx.x <= R && threadIdx.y <= R2)
  {
    ...
    data[threadIdx.x + __mul24(threadIdx.y, S2) + R] =
      kernel[threadIdx.x + __mul24(threadIdx.y, S2) + R];
    data[threadIdx.x + __mul24(threadIdx.y, S2) + S] =
      kernel[threadIdx.x + __mul24(threadIdx.y, S2) + S];
    data[threadIdx.x + __mul24(threadIdx.y, S2) + S + R] =
      kernel[threadIdx.x + __mul24(threadIdx.y, S2) + R + S];
  }
  __syncthreads();
#endif
  ...
  {
    // The shared-memory tile staging reserved by shared_size in convolve()
    // is unused; the inner loop reads `input` from global memory directly.
    int x, y;
    x = 2 * threadIdx.x;
    y = threadIdx.y;
    ...
    for (int r = -R; r <= R; ++r)
    {
      for (int s = -R; s <= R - 1; s += 2)
      {
        ...
        float2 kernel2 =
          reinterpret_cast<const float2 *>(data)[(__mul24(r + R, S2) + s + R) / 2 + 1 + R];
#else
        float2 data2 = reinterpret_cast<const float2 *>(input)[(gindex + r * W + s) / 2 + 1];
        float2 kernel2 =
          reinterpret_cast<const float2 *>(kernel)[(__mul24(r + R, S2) + s + R) / 2 + 1 + R];
#endif
        sum.y = fma(data2.x, kernel2.x, sum.y);
        sum.y = fma(data2.y, kernel2.y, sum.y);
      }
#ifdef CONV_SHARED
      sum.x += input[gindex + r * W + R] * data[__mul24(r + R, S2) + R2];
      sum.y += input[gindex + r * W - R + 1] * data[__mul24(r + R, S2)];
#else
      sum.x += input[gindex + r * W + R] * kernel[__mul24(r + R, S2) + R2];
      sum.y += input[gindex + r * W - R + 1] * kernel[__mul24(r + R, S2)];
#endif
    }
...
template <typename T>
__global__ void nonMaxSuppress_k(
  T *dst, const T *src, int rows, int cols, int R, int stride, int pad
)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int j = blockIdx.y * blockDim.y + threadIdx.y;
...
template <typename T>
__global__ void min_k(T *dst, const T *src1, const T *src2, int rows, int cols)
{
...
template <typename T>
__global__ void min4_k(
  T *dst, const T *src1, const T *src2, const T *src3, const T *src4, int rows, int cols
)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  {
    int index = i * cols + j;
    dst[index] = min(min(src1[index], src2[index]), min(src3[index], src4[index]));
  }
}

template <typename T>
__global__ void max_k(T *dst, const T *src1, const T *src2, int rows, int cols)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < rows && j < cols)
...
template <typename T>
__global__ void sub_k(T *dst, const T *src1, const T *src2, int rows, int cols)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < rows && j < cols)
...
template <typename T>
__global__ void combineLikelihood_k(
  T *dst, const T *A, const T *B, const T *C, const T *D, int rows, int cols
)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename T>
__global__ void bgr_to_gray_interleaved_k(T *dst, const T *src, int width, int height)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < height && j < width)
...
template <typename T>
__global__ void bgr_to_gray_planar_k(T *dst, const T *src, int width, int height)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < height && j < width)
  {
    ...
    dst[b_index] = 0.299 * src[r_index] + 0.587 * src[g_index] + 0.114 * src[b_index];
  }
}
...
template <typename T>
__global__ void gaussian9x9_k(T *dst, const T *src, float sigma, int rows, int cols)
{
  int i = blockIdx.y * blockDim.y + threadIdx.y;
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < rows && j < cols)
...
template <typename T>
__device__ Point<T> smallestEigenvector(T *A)
{
  T a = A[0];
  T b = A[1];
  T c = A[2];
...
__device__ bool smoothHistogram(T *bins, int bin_size, float sigma)
{
  ...
  {
    bins[i] = smoothed[i];
    if (fabsf(bins[i] - bins[0]) > 0.0001) nonzero = true;
  }
...
template <typename T>
__global__ void cornerEdges_k(
  T *corners_edge1_sin, T *corners_edge1_cos, T *corners_edge2_sin, T *corners_edge2_cos,
  int num_corners, const T *gx, const T *gy, const T *corners_x, const T *corners_y, int radius,
  ...
  if (i < num_corners)
  {
    ...
    for (int r = -radius; r <= radius; ++r)
    {
      for (int s = -radius; s <= radius; ++s)
      {
        ...
        {
          T ga = atan2(gy[y_index * width + x_index], gx[y_index * width + x_index]);
...
template <typename T>
__global__ void refine_corners_k(
  T *corners_x, T *corners_y, int num_corners, const T *corners_edge1_sin,
  const T *corners_edge1_cos, const T *corners_edge2_sin, const T *corners_edge2_cos, const T *gx,
  const T *gy, int radius, int width, int height
)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < num_corners)
...
__global__ void gradientScore_k(
  float *score, const float *gx, const float *gy, const float *x, const float *y, const float *sin1,
  const float *cos1, const float *sin2, const float *cos2, int radius, int num_corners, int width,
  int height
)
{
}
...
void pad(GpuImagePtr &dst, const GpuImagePtr &src, int pad)
{
  ...
  padBorder_k<float><<<grid, block>>>(dst->data(), src->data(), src->height(), src->width(), pad);
}
...
void nms(GpuImagePtr &dst, const GpuImagePtr &src, int R, int stride, int pad)
{
  ...
  nonMaxSuppress_k<float>
    <<<grid, block>>>(dst->data(), src->data(), src->height(), src->width(), R, stride, pad);
}
...
void CudaConvolver::convolve(
  const GpuImagePtr &image, const GpuKernelPtr &kernel, GpuImagePtr &output
)
{
  ...
  const dim3 grid((W + block.x * 2 - 1) / (2 * block.x), (H + block.y - 1) / block.y);
  const size_t shared_size = (block.x * 2 + 2 * pad) * (block.y + 2 * pad) * sizeof(float);

#ifndef NDEBUG
  std::cout << "convolving: " << image->width() << "x" << image->height() << std::endl;
#endif
...
#ifndef NDEBUG
  std::cout << "convolved: " << output->width() << "x" << output->height() << std::endl;
#endif
}
...
static void get_corners_edge(
  GpuImagePtr &corners_edge1_sin, GpuImagePtr &corners_edge1_cos, GpuImagePtr &corners_edge2_sin,
  GpuImagePtr &corners_edge2_cos, int num_corners, const GpuImagePtr &gx, const GpuImagePtr &gy,
  const GpuImagePtr &corners_x, const GpuImagePtr &corners_y, int radius, int width, int height,
  bool refine
)
{
  constexpr int block_size = 32;
  cornerEdges_k<float><<<num_corners / block_size + 1, block_size>>>(
    corners_edge1_sin->data(), corners_edge1_cos->data(), corners_edge2_sin->data(),
    corners_edge2_cos->data(), num_corners, gx->data(), gy->data(), corners_x->data(),
    corners_y->data(), radius, width, height, refine
  );
}

static void refine_corners(
  GpuImagePtr &corners_x, GpuImagePtr &corners_y, int num_corners,
  const GpuImagePtr &corners_edge1_sin, const GpuImagePtr &corners_edge1_cos,
  const GpuImagePtr &corners_edge2_sin, const GpuImagePtr &corners_edge2_cos, const GpuImagePtr &gx,
  const GpuImagePtr &gy, int radius, int width, int height
)
{
  constexpr int block_size = 32;
  refine_corners_k<float><<<corners_x->size() / block_size + 1, block_size>>>(
    corners_x->data(), corners_y->data(), num_corners, corners_edge1_sin->data(),
    corners_edge1_cos->data(), corners_edge2_sin->data(), corners_edge2_cos->data(), gx->data(),
...
static GpuImagePtr sobel_x(const GpuImagePtr &src)
{
  ...
  gradX_k<float><<<grid, block>>>(dst->data(), src->data(), src->height(), src->width());
...
static GpuImagePtr sobel_y(const GpuImagePtr &src)
{
  ...
  gradY_k<float><<<grid, block>>>(dst->data(), src->data(), src->height(), src->width());
...
void gradients(GpuImagePtr &gx, GpuImagePtr &gy, const GpuImagePtr &src)
{
  ...
  gradX_k<float><<<grid, block>>>(gx->data(), src->data(), src->height(), src->width());
  gradY_k<float><<<grid, block>>>(gy->data(), src->data(), src->height(), src->width());
}
...
void weight(GpuImagePtr &dst, const GpuImagePtr &gx, const GpuImagePtr &gy)
{
  ...
  dim3 block(32, 24);
  dim3 grid((gx->width() + block.x - 1) / block.x, (gx->height() + block.y - 1) / block.y);

  gradientWeight_k<float>
    <<<grid, block>>>(dst->data(), gx->data(), gy->data(), gx->height(), gx->width());
}

CudaRefiner::CudaRefiner() { NVCHK(cudaStreamCreate(&stream_)); }
CudaRefiner::~CudaRefiner() { NVCHK(cudaStreamDestroy(stream_)); }

void CudaRefiner::refine(
  const GpuImagePtr &image, const GpuImagePtr &gx, const GpuImagePtr &gy, CornerArray &corners,
...
)
{
  ...
  corners_x->resize(corners.x.size(), 1);
  ...
  get_corners_edge(
    corners_edge1_sin, corners_edge1_cos, corners_edge2_sin, corners_edge2_cos, corners.x.size(),
    gx, gy, corners_x, corners_y, radius, image->width(), image->height(), redges
  );
  ...
  refine_corners(
    corners_x, corners_y, corners.x.size(), corners_edge1_sin, corners_edge1_cos, corners_edge2_sin,
    corners_edge2_cos, gx, gy, radius, image->width(), image->height()
  );
...
void min(const GpuImagePtr &src1, const GpuImagePtr &src2, GpuImagePtr &dst)
{
  ...
  dim3 block(32, 24);
  dim3 grid((src1->width() + block.x - 1) / block.x, (src1->height() + block.y - 1) / block.y);

  min_k<float>
    <<<grid, block>>>(dst->data(), src1->data(), src2->data(), src1->height(), src1->width());
}

void min4(
  const GpuImagePtr &src1, const GpuImagePtr &src2, const GpuImagePtr &src3,
...
)
{
  ...
  min4_k<float><<<grid, block>>>(
    dst->data(), src1->data(), src2->data(), src3->data(), src4->data(), src1->height(),
    src1->width()
  );
}
...
void neg(const GpuImagePtr &src, GpuImagePtr &dst)
{
  ...
  neg_k<float><<<grid, block>>>(dst->data(), src->data(), src->height(), src->width());
}
...
)
{
  ...
  dim3 block(32, 24);
  dim3 grid((A->width() + block.x - 1) / block.x, (A->height() + block.y - 1) / block.y);

  combineLikelihood_k<float><<<grid, block>>>(
    dst->data(), A->data(), B->data(), C->data(), D->data(), A->height(), A->width()
  );
}
...
void scoreGradient(CornerArray &corners, const GpuImagePtr &src, int radius)
{
  ...
  gradientScore_k<<<grid, block>>>(
    score->data(), gx->data(), gy->data(), corners_x->data(), corners_y->data(),
    corners_edge1_sin->data(), corners_edge1_cos->data(), corners_edge2_sin->data(),
    corners_edge2_cos->data(), radius, corners.x.size(), src->width(), src->height()
  );
...
void bgrToGray(T *dst, const T *src, int width, int height, bool interleaved)
{
  ...
  if (interleaved)
    bgr_to_gray_interleaved_k<T><<<grid, block>>>(dst, src, width, height);
  else
    bgr_to_gray_planar_k<T><<<grid, block>>>(dst, src, width, height);
}
...
void gaussian9x9(T *dst, const T *src, float sigma, int width, int height)
{
  ...
  gaussian9x9_k<T><<<grid, block>>>(dst, src, sigma, width, height);
}
...
void normalize(float *dst, const unsigned char *src, int width, int height)
{
  ...
  // std::cout << "min: " << min << ", max: " << max << std::endl;
  normalize_k<unsigned char><<<grid_size, block_size>>>(dst, src, min, max, width * height);
  delete[] h_blocks_max;
  delete[] h_blocks_min;
...
template void gaussian9x9<float>(float *dst, const float *src, float sigma, int width, int height);
template void gaussian9x9<unsigned char>(
  unsigned char *dst, const unsigned char *src, float sigma, int width, int height
);
