...
void CudaLikelihoodEstimator::configureKernels(int radius, float angle1, float angle2)
{
  for (int i = 0; i < 4; ++i)
  {
...
GpuImagePtr CudaLikelihoodEstimator::buildMap(const GpuImagePtr &image)
{
  ...
  cuda::likelihood(
    likelihood_map_, convolved_images_[0], convolved_images_[2], convolved_images_[1],
    convolved_images_[3]
  );
...
CudaDetector::CudaDetector(
  ...
  bool verbose_logging, bool use_separable, int separable_rank
)
{
  impl_ =
    new CudaDetectorImpl(radius, angle1, angle2, verbose_logging, use_separable, separable_rank);
}

CudaDetector::~CudaDetector() { delete impl_; }

CornerArray CudaDetector::detect(
  const float *data, int width, int height, DetectThresholds s, bool convert_to_gray
)
{
#ifdef TIME_LOGGER
  auto start = TimeNow();
...
#endif
  return impl_->runDetect(input_, s.nms_margin, s.nms_radius, s.nms_threshold, s.score_threshold);
}

CornerArray CudaDetector::detect(const GpuImagePtr &image, DetectThresholds s)
{
  return impl_->runDetect(image, s.nms_margin, s.nms_radius, s.nms_threshold, s.score_threshold);
}
...
CornerArray CudaDetectorImpl::detectHost(
  const unsigned char *data, int width, int height, int channels, DetectThresholds s
)
{
#ifdef TIME_LOGGER
  ...
  std::cout << "Preprocess (GPU, incl upload) time: " << TimeDiff(pstart, TimeNow()) * 1e-3 << " ms"
            << std::endl;
#endif
  return runDetect(norm, s.nms_margin, s.nms_radius, s.nms_threshold, s.score_threshold);
}

CornerArray CudaDetectorImpl::runDetect(
...
)
{
...
#ifdef TIME_LOGGER
  auto end = TimeNow();
  std::cout << "Map generation time: " << TimeDiff(start, end) * 1e-3 << " ms" << std::endl;
#endif
  ...
  std::cout << "Pruning time: " << TimeDiff(start, end) * 1e-3 << " ms" << std::endl;
#endif
  return corners;
}

void CudaDetectorImpl::pruneCorners(
  const GpuImagePtr &devimg, const GpuImagePtr &weights, CornerArray &corners, float threshold
)
{
  for (int i = 0; i < corners.x.size(); ++i)
  {
    if (corners.edge1_cos[i] == 0 && corners.edge1_sin[i] == 0 && corners.edge2_cos[i] == 0 &&
        corners.edge2_sin[i] == 0)
    {
      corners.x[i] = 0;
      corners.y[i] = 0;
    }
  ...
  for (int i = 0; i < corners.x.size(); ++i)
  {
    ...
    {
      if (fabsf(corners.x[i] - corners.x[j]) < 1.0 && fabsf(corners.y[i] - corners.y[j]) < 1.0)
      {
        corners.x[j] = 0;
        corners.y[j] = 0;
  ...
  for (int i = 0; i < corners_n1.size(); i++)
  {
    ...
    float flipflag =
      corners_n1[i][0] * corners.edge2_cos[i] + corners_n1[0][1] * corners.edge2_sin[i];
    if (flipflag > 0)
      flipflag = -1;
    else
      flipflag = 1;
