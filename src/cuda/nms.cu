...
__global__ void nms_compact_kernel(const float * __restrict__ src,
                                   int rows, int cols, int radius,
                                   int margin, float threshold,
                                   int * __restrict__ d_out_x,
                                   int * __restrict__ d_out_y,
...
void CudaNonMaxSuppression::ensureCapacity(std::size_t min_capacity) {
  ...
  d_out_y_ = nullptr;
  d_out_v_ = nullptr;
  checkCuda(cudaMalloc(&d_out_x_, min_capacity * sizeof(int)),
            "alloc d_out_x");
  checkCuda(cudaMalloc(&d_out_y_, min_capacity * sizeof(int)),
            "alloc d_out_y");
  checkCuda(cudaMalloc(&d_out_v_, min_capacity * sizeof(float)),
            "alloc d_out_v");
  capacity_ = min_capacity;
}

std::size_t CudaNonMaxSuppression::run(const GpuImagePtr & likelihood_map,
  ...
  nms_compact_kernel<<<grid, block>>>(
      likelihood_map->data(), rows, cols, radius, margin, threshold,
      d_out_x_, d_out_y_, d_out_v_, d_count_,
      static_cast<int>(capacity_));
  checkCuda(cudaGetLastError(), "launch nms_compact_kernel");

  int h_count = 0;
  checkCuda(cudaMemcpy(&h_count, d_count_, sizeof(int),
  ...
  checkCuda(cudaMemcpy(h_x.data(), d_out_x_, kept * sizeof(int),
                       cudaMemcpyDeviceToHost),
            "copy d_out_x");
  checkCuda(cudaMemcpy(h_y.data(), d_out_y_, kept * sizeof(int),
                       cudaMemcpyDeviceToHost),
            "copy d_out_y");
  checkCuda(cudaMemcpy(h_v.data(), d_out_v_, kept * sizeof(float),
                       cudaMemcpyDeviceToHost),
            "copy d_out_v");
