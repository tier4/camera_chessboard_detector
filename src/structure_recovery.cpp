...
constexpr double kPredictStep =
  0.75;  // fraction of the extrapolated step used to place a prediction
...
int BoardBuilder::nearestNeighborAlong(
  int idx, cv::Vec2f v, cv::Mat chessboard, CornerArray &corners, int &neighbor_idx, float &min_dist
)
{
...
cv::Mat BoardBuilder::seedBoard(CornerArray &corners, int idx)
{
  ...
  nearestNeighborAlong(idx, +1 * v1, chessboard, corners, chessboard.at<int>(1, 2), dist1[0]);
  nearestNeighborAlong(idx, -1 * v1, chessboard, corners, chessboard.at<int>(1, 0), dist1[1]);
  nearestNeighborAlong(idx, +1 * v2, chessboard, corners, chessboard.at<int>(2, 1), dist2[0]);
  nearestNeighborAlong(idx, -1 * v2, chessboard, corners, chessboard.at<int>(0, 1), dist2[1]);

  // place the four diagonal neighbours

  nearestNeighborAlong(
    chessboard.at<int>(1, 0), -1 * v2, chessboard, corners, chessboard.at<int>(0, 0), dist2[2]
  );
  nearestNeighborAlong(
    chessboard.at<int>(1, 0), +1 * v2, chessboard, corners, chessboard.at<int>(2, 0), dist2[3]
  );
  nearestNeighborAlong(
    chessboard.at<int>(1, 2), -1 * v2, chessboard, corners, chessboard.at<int>(0, 2), dist2[4]
  );
  nearestNeighborAlong(
    chessboard.at<int>(1, 2), +1 * v2, chessboard, corners, chessboard.at<int>(2, 2), dist2[5]
  );
  ...
  reject = reject || (dist2[0] < 0) || (dist2[1] < 0) || (dist2[2] < 0) || (dist2[3] < 0) ||
           (dist2[4] < 0) || (dist2[5] < 0);

  reject =
    reject || (coeffVariation(dist1) > kMaxSpacingCV) || (coeffVariation(dist2) > kMaxSpacingCV);
...
float BoardBuilder::boardEnergy(cv::Mat chessboard, CornerArray &corners)
{
  ...
  for (int i = 0; i < chessboard.rows; i++)
    for (int j = 0; j < chessboard.cols - 2; j++)
    {
      ...
      if (energy_structure < triple_energy) energy_structure = triple_energy;
    }
...
void BoardBuilder::predictNextCorners(
...
void BoardBuilder::matchClosestCandidates(
  std::vector<cv::Vec2f> &candidates, std::vector<cv::Vec2f> &predicted, std::vector<int> &idx
)
{
...
cv::Mat BoardBuilder::growBoard(cv::Mat chessboard, CornerArray &corners, int border_type)
{
    ...
    case 0: {
      ...
      for (int row = 0; row < chessboard.rows; row++)
        for (int col = 0; col < chessboard.cols; col++)
        {
          ...
          {
            p1.push_back(cv::Point2f(
              corners.x[chessboard.at<int>(row, col)], corners.y[chessboard.at<int>(row, col)]
            ));
          }
          if (col == chessboard.cols - 2)
          {
            p2.push_back(cv::Point2f(
              corners.x[chessboard.at<int>(row, col)], corners.y[chessboard.at<int>(row, col)]
            ));
          }
          if (col == chessboard.cols - 1)
          {
            p3.push_back(cv::Point2f(
              corners.x[chessboard.at<int>(row, col)], corners.y[chessboard.at<int>(row, col)]
            ));
          }
        }
...
void BoardBuilder::assembleBoards(
  CornerArray &corners, std::vector<cv::Mat> &chessboards, float lambda
)
{
  if (verbose_logging_)
  {
  ...
  for (int i = 0; i < corners.x.size(); i++)
  {
    ...
    if (boardEnergy(chessboard, corners) < kAcceptEnergy)
    {
      ...
      for (int j = 0; j < chessboards.size(); j++)
      {
        ...
        {
          int existing_cell =
            chessboards[j].at<int>(k / chessboards[j].cols, k % chessboards[j].cols);
          for (int l = 0; l < chessboard.size().area(); l++)
          {
            int new_cell = chessboard.at<int>(l / chessboard.cols, l % chessboard.cols);
